...
class TripletOxfordFlowers(Dataset):
    """
    ...
    def __init__(self, root, train=True, transform=None):
        ...
        self.labels_set = set(self.data.target)
        self.label_to_indices = {label: np.where(self.data.target == label)[0]
                                 for label in self.labels_set}

    def __getitem__(self, idx):
        if self.transform is not None:
            if isinstance(idx, int) and self.is_train:

                sample = self.data.iloc[idx]
                anchor_path = os.path.join(self.root, sample.img_path)
                anchor_label = sample.target
                anchor = self.loader(anchor_path)

                positive_idx = idx
                while positive_idx == idx:
                    positive_idx = random.choice(self.label_to_indices[anchor_label])
                positive_sample = self.data.iloc[positive_idx]
                positive_path = os.path.join(self.root, positive_sample.img_path)
                positive = self.loader(positive_path)

                negative_label = random.choice(list(self.labels_set - {anchor_label}))
                negative_idx = random.choice(self.label_to_indices[negative_label])
                negative_sample = self.data.iloc[negative_idx]
                negative_path = os.path.join(self.root, negative_sample.img_path)
				...
				#must ensure B%3 == 0
                return (anchor, positive, negative), (anchor_label, positive_sample.target, negative_sample.target)
