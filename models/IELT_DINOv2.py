...
from models.modules_dinov2 import *
...
class IELT_DINOv2(nn.Module):
	...
	def load_from(self, weights):
		with torch.no_grad():
			...
			self.embeddings.patch_embeddings.weight.copy_(weights["patch_embed.proj.weight"])
			self.embeddings.patch_embeddings.bias.copy_(weights["patch_embed.proj.bias"])
			#self.embeddings.position_embeddings.copy_(weights["pos_embed"])
			self.embeddings.cls_token.copy_(weights["cls_token"])
			...
			for idx, block in enumerate(self.encoder.layer):
				block.load_from(weights, n_block=idx, dinov2=True)
